...
function HelpOverlay({ text, position = 'bottom' }) {
  // 기본 하단 중앙. S5처럼 하단에 목표물(지문확인)이 있으면 'top'으로 올려 가리지 않게(FIX-J).
  const vpos = position === 'top' ? 'top-6' : 'bottom-6'
  return (
    <div className={`pointer-events-none absolute inset-x-0 ${vpos} flex justify-center px-6`}>
